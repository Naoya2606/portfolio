...
export function ArtistFileList({ artistId }: ArtistFileListProps) {
  const { data, mutate } = useSWR(`/api/artists/${artistId}/files`, fetcher);
  const [uploading, setUploading] = useState(false);
  const [error, setError] = useState("");
  const [deleteTarget, setDeleteTarget] = useState<ArtistFile | null>(null);
  const [deleting, setDeleting] = useState(false);
  const [dragOver, setDragOver] = useState(false);
  const fileInputRef = useRef<HTMLInputElement>(null);

  const files: ArtistFile[] = data?.data || [];
  ...
  async function handleDelete() {
    ...
    try {
      await fetch(`/api/artists/${artistId}/files/${deleteTarget.id}`, {
        method: "DELETE",
      });
      mutate();
    } catch {
